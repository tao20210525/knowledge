...
public class KnowledgeInfoDaoImpl implements KnowledgeInfoDao {
    ...
    @Override
    public List<Map<String, Object>> getKnowledgeTemplateInfo() {
        List<Map<String, Object>> returnList = null;
        try {
            List<Object[]> resultList = null;
            StringBuffer sql = new StringBuffer();
            sql.append("call findKnowledge()");
            Query query = this.entityManager.createNativeQuery(sql.toString());
            resultList = query.getResultList();
            if (null != resultList && resultList.size() > 0) {
                returnList = new ArrayList<Map<String, Object>>();
                for (Object[] objArr : resultList) {
                    Map<String, Object> elementInfo = new HashMap<String, Object>();
                    elementInfo.put("id", null == objArr[0] ? 0L : objArr[0].toString());
                    elementInfo.put("status", null == objArr[1] ? "" : objArr[1].toString());
                    for (int i = 2; i < objArr.length; i++) {
                        String key = i - 1 + "";
                        elementInfo.put(key, null == objArr[i] ? "" : objArr[i].toString());
                    }
                    returnList.add(elementInfo);
                }
            }
